...
To illustrate the problem, I present a simple example. Assume the following regression equation for a continuous variable, $z_i$: $z_i = x_i + \epsilon_i$, where $i=1, 2, ..., 5000$, $x_i\sim\mathcal{N}(0, 1)$ and $\epsilon_i\sim\mathcal{L}\big(0, 0.5 + \frac{1}{4}\sqrt{e^{x_i}}\big)$, so the error variance is non-constant and depends on $x_i$. I dichotomize $z_i$ at 0 to create a new binary response, $y_i$, such that $y_i = 1$ where $z_i > 0$ and $y_i = 0$ when $z_i \leq 0$. So $z_i$ is the continuous variable underlying the binary outcome, $y_i$. For comparison, I also generated a homoskedastic $z_i$ and $y_i$ under the condition that $\epsilon_i\sim\mathcal{L}(0, 1)$. As is visible from Figure \@ref(fig:fig1), the average relationship between $x_i$ and each $z_i$ is not that different under homoskedasticity and heteroskedasticity, but the heteroskedastic $z_i$ visibly displays non-constant error variance.

```{r fig1, fig.cap = "Relationship between x and continuous outcome under heteroskedasticity (left panel) and homoskedasticity (right panel). Under heteroskedasticity, the variance of z is larger at higher values of x.", fig.height = 3}
n <- 5000
set.seed(n + 5)
dat1 <- data.frame(ID = 1:n, x = rnorm(5000))
dat1 <- within(dat1, { het = rlogis(n, 0, .5 + sqrt(exp(x)) / 4); hom = rlogis(n) })
dat1 <- within(dat1, { zhet <- x + het; zhom <- x + hom })
dat1 <- within(dat1, { yhet <- (zhet > 0) + 0; yhom <- (zhom > 0) + 0 })
dat1l <- tidyr::gather(dat1, rel, score, zhom:yhet)
...
ggplot(dat1l[grepl("z", dat1l$rel), ], aes(x, score)) + facet_wrap(~ rel1) + geom_point(shape = 1) +
  geom_smooth(method = "lm") + theme_bw() + labs(y = "z")
```

Next, I regressed the continuous variable, $z_i$, on $x_i$ using linear regression. Regardless of the error variance structure, the linear regression model had a decent recovery of the coefficient for $x_i$; both coefficients were quite close to 1 (see first two columns of Table 1). This is consistent with the theory on linear regression models: unbiased coefficient estimation does not dependent on assumptions like homoskedasticity or normality of errors [@Gelman2007, p. 46].
...
I next regressed the binary variable, $y_i$, on $x_i$ using logistic regression. Under homoskedastic error variance, the coefficient of $x_i$ was 1.027, close to 1. However, under heteroskedastic error variance, the coefficient of $x_i$ was about 30% higher $\big((1.316-1)\times 100\%\big)$ than what I specified in the data generation process. In this situation, we have inflation of the coefficient. Depending on the form of heteroskedasticity, the result might be coefficient deflation.
